...
/*!
 * Makes a string safe for a regular expression. Mirrors swig-django's parser.
 * @private
 */
 ...
 * Django parser builds at parse-time so the pre-walker chunks the same way
...
/*!
 * Strip tag controls from a tag chunk, returning the trimmed tag body (e.g.
 * <code>extends "x.html"</code>). Django has no whitespace-control markers,
 * so there is nothing to strip beyond the controls + surrounding space.
 * @private
 */
...
/**
 * Scan Django template source for static <code>{% extends|include "..." %}</code>
 * targets. Pure function; performs no I/O.
 *
 * The scanner mirrors the Django parser's chunk-splitter so it agrees on
 ...
 *
 * @example
 * preWalker.scan('{% include "partial.html" %}', {
 *   varControls: ['{{', '}}'],
 *   tagControls: ['{%', '%}'],
 *   cmtControls: ['{#', '#}'],
 *   rawTag: 'verbatim',
 ...
 *                                    (<code>verbatim</code> for Django).
 * @param  {array}  opts.keywords     Keywords whose first quoted argument
 *                                    is a template path. Django:
 *                                    <code>['extends', 'include']</code>.
