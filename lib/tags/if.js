...
exports.compile = function (compiler, args, content, parents, options, blockName, token) {
  // Phase 2 Session 14c: scan content for `else` / `elseif` peer tokens
  // and build a multi-branch IRIf. Each elseif's test is taken from its
  // own `token.irExpr` (attached by elseif.lowerExpr) when available,
  // else falls back to the legacy `args.join(' ')` JS string — same
  // per-branch dispatch the backend If emitter applies for the first
  // branch. Body slices between markers are compiled via the standard
  // walker and wrapped in an IRLegacyJS node.
  var branches = [],
    currentTest = (token && token.irExpr) ? token.irExpr : args.join(' '),
    currentSlice = [],
    i,
    t;
  for (i = 0; i < content.length; i++) {
    ...
    if (t && (t.name === 'elseif' || t.name === 'else')) {
      ...
      if (t.name === 'else') {
        ...
        currentTest = (t.irExpr) ? t.irExpr : t.args.join(' ');
      }
      currentSlice = [];
    } else {
      currentSlice.push(t);
