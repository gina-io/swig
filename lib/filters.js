 ...
 *
 * The output is HTML-escaped (`<`, `>`, `&`, `'`) and marked safe, so it is not
 * re-escaped by autoescape and is safe to embed directly inside a `<script>` block.
 *
 * @example
 * // val = { a: 'b' }
 * {{ val|json }}
...
exports.json = function (input, indent) {
  ...
  if (typeof json !== 'string') {
    // JSON.stringify returns undefined for functions / symbols / undefined input;
    // pass it through (prior behavior) rather than throwing on .replace below.
    return json;
  }
  // Self-escape the HTML-significant characters JSON.stringify leaves intact
  // (<, >, &, ') so the output is safe to embed directly in a <script> block and
  // cannot break out of it. Marked .safe below so autoescape does not re-escape it.
  // Mirrors the swig-jinja2 `tojson` filter.
  return json
    .replace(/</g, '\\u003c')
    .replace(/>/g, '\\u003e')
    .replace(/&/g, '\\u0026')
 ...
 *
 * Output is autoescaped (the filter is intentionally not marked safe), because
 * decoded content can contain HTML-significant characters.
 *
 * @example
