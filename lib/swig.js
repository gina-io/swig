var utils = require('./utils'),
  ...
  cache = require('../packages/swig-core/lib/cache'),
  engine = require('../packages/swig-core/lib/engine');

/**
 * Swig version number as a string.
 * @example
 * if (swig.version === "1.6.0") { ... }
 *
 * @type {String}
 */
...
exports.Swig = function (opts) {
   ...
   * Thin wrapper around swig-core engine.getParents that injects this
   * instance's loader + parseFile + cacheGet as dependencies.
   *
  ...
  function getParents(tokens, options) {
    return engine.getParents(tokens, options, {
  ...
  this.precompile = function (source, options) {
    ...
    if (parents.length) {
      ...
      tokens.tokens = engine.remapBlocks(tokens.blocks, parents[0].tokens);
      engine.importNonBlocks(tokens.blocks, tokens.tokens);
    }

    try {
      tpl = engine.buildTemplateFunction(tokens, parents, options);
    } catch (e) {
